...
# plotting analysis results, pairwise comparison and range plotting ------
  ...
  plot_analysis_factor <- function(analysis_object, signif_digits = 3, 
                                   ...
                                   cust_theme = NULL, label_txt_size = 2.6) {
    ...
    plot_tag <- analysis_object$stat_tables %>% 
      map2_chr(., names(.), function(x, y) paste(y, ': n = ', x$total_n[1], sep = '')) %>% 
      paste(collapse = '\n') %>% 
      paste('\n', ., sep = '')
  ...
  plot_analysis_numeric <- function(analysis_object, signif_digits = 3, 
                                    ...
                                    cust_theme = NULL, point_alpha = 0.3, y_transf = 'identity') {
    ...
    plot_tag <- analysis_object$stat_tables %>% 
      map2_chr(., names(.), function(x, y) paste(y, ': n = ', x$n_complete[1], sep = '')) %>% 
      paste(collapse = '\n') %>% 
      paste('\n', ., sep = '')
